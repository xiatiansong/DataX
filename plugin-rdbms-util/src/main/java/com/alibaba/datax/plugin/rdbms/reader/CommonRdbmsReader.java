                ...
                for (int i = 1; i <= columnNumber; i++) {
                    switch (metaData.getColumnType(i)) {
                        ...
                        case Types.TIMESTAMP:
                            if (dataBaseType == DataBaseType.PostgreSQL && rs.getObject(i) != null) {
                                record.addColumn(new DateColumn(rs.getObject(i, LocalDateTime.class).atZone(ZoneId.of("UTC+0")).toInstant().toEpochMilli()));
                            } else {
